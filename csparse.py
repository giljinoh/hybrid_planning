...
def CS_TRIPLET(A):
    ...
    #C = cs()
    #C = cs_spalloc(A.m,A.n,A.nz,A.x != None, 0)
    #print(C.nz)
    #return C
    # if  C != None and C.nz >= 0:
    #     return C
    # else:
    #     return -1
...
def cs_triplet(A):
    ...
    C = cs_spalloc(m,n,nz,Ax,Ax != None, 0)
    w = [None] * n
    if not C or not w:
        return C
    Cp = C.p
...
def cs_amd(order, A):
    ...
    # for i in range(1401):
        # print(elen[elen_offset+ i], end = "")
    # print(Cp)
    # for i in range(1401):
    #     print("nvj" ,nv[nv_offset + i],end=" ")
    # for i in range(1401):
    #     print("nect" ,next[next_offset + i],end=" ")
    while nel < n: # while (selecting pivots) do
        # for i in range(1401):
        #     print(" ", head[head_offset+ i], end = "   ")
        # print("")
        # print(Ci)
        ...
        while mindeg < n and k == -1:
            ...
            mindeg += 1
        mindeg -= 1
        # print(" min ",mindeg, end = " ")
        # print("      ", end = "")
        # print("k",head[head_offset + mindeg] , end="" )
        # print( k, end = "  ")
        ...
        # print("k ",nel, nvk , end=" " )
        # print( next[next_offset + 802])
        # for i in range(5):
        #     print(" " ,head[head_offset + mindeg], mindeg,end="   ")
        # for i in range(9510):
        #     print(" ", Ci[i], end = " ")
        # print("\n")
        # --- Garbage collection -------------------------------------------
        if elenk > 0 and cnz + mindeg >= nzmax:
            #print("jashdkjahdkjahkdjhakjdhkjasdh")
            for j in range(n):
                p = Cp[j]
        ...
        if elenk == 0:
            ...
            pk1 = cnz # do in place if elen[elen_offset+k] == 0
        pk2 = pk1
        # print("i", pk1, pk2,end = " ")
        # print(k, Cp[k], end = " ")
        # for i in range(1401):
        #     print(" " ,nv[nv_offset + i],end=" ")
        # print("\n")
        # print("ele", elenk+1, end="")
        for k1 in range(1, elenk + 2): #1): #2): --------------------------------------------
            if k1 > elenk:
        ...
        # print("\n")
        # --- Degree update ------------------------------------------------
        # print("k3 ",nel, nvk , end=" " )
        #for pk in range(pk2): # scan2: degree update ------------------------------------------------------------------------
        for pk in range(pk1, pk2): # scan2: degree update    
            ...
            for p in range(p2 + 1, p4): # prune edges in Ai
                # print(" ", p2, end=" ")
                j = Ci[p]
                # print("nvj" ,nv[nv_offset + j],end=" ")
                nvj = nv[nv_offset + j]
...
def cs_lu(A, S, tol):
    ...
    for k in range(n): # compute L(:,k) and U(:,k)
        ...
        Lx[lnz] = 1
        lnz+=1
        for p in range(top, n): # L(k+1:n,k) = x / pivot
            i = xi[p]
